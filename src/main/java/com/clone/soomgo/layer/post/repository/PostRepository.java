...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    Slice<Post> findAllByTitleContainingOrContentContainingOrTagsContainingOrderByIdDesc(String title,String content,String tags,Pageable pageable);


    @Query("select p from Post p where p.id<?1 AND (p.title LIKE %?2% Or p.tags LIKE %?3% Or p.content LIKE %?4%) ORDER BY p.id DESC ")
    Slice<Post> findByIdLessThanAndTitleContainingOrContentContainingOrderByIdDesc(Long id,String title,String tags,String content, Pageable pageable);
